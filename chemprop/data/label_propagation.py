...
def adjacency_matrix(data, model, method, batch_size=64):
    matrix = np.zeros((len(data), len(data))) # diagonal entries should be 0
    if method == 'embedding':
        vecs = compute_molecule_vectors(model=deepcopy(model), data=data, batch_size=batch_size)
...
def propagate_labels(model, data, transductive_data, args): 
    ...
    assert 0 <= args.label_prop_alpha and args.label_prop_alpha < 1
